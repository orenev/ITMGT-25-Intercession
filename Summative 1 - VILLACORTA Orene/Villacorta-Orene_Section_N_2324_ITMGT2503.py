...
new_jsondata = {key.upper():str(value.upper()) for key,value in jsondata.items()}
new_jsondata_noP = {key:value.replace("<P>",'') for key,value in new_jsondata.items()}
new_jsondata_noP_slash = {key:value.replace("</P>",'') for key,value in new_jsondata_noP.items()}
new_jsondata_noB = {key:value.replace("<B>",'') for key,value in new_jsondata_noP_slash.items()}
new_jsondata_noB_slash = {key:value.replace("</B>",'') for key,value in new_jsondata_noB.items()}
new_jsondata_noI = {key:value.replace("<I>",'') for key,value in new_jsondata_noB_slash.items()}
new_jsondata_removed = {key:value.replace("</I>",'') for key,value in new_jsondata_noI.items()}
new_jsondata_punctuation = {key:value.replace(i,' ') for key,value in new_jsondata_removed.items() for i in unnecessary_punctuation}
print(new_jsondata_punctuation)
...
for key,value in new_jsondata_punctuation.items():
    for word in value:
        if len(word) >= 3: 
            wordcount_dictionary.update([(key,word)])
...
optimal_Z = 10*optimal_F + 12*optimal_R + 750*optimal_C
optimal_R = max((18000 - 1000*optimal_F - 750*optimal_C) // 600, (12000-500*optimal_F-300*optimal_C)//750, (1500-50*optimal_F-75*optimal_C)//45)
optimal_F = max((18000-750*optimal_C-600*optimal_R)//1000, (12000-750*optimal_R-300*optimal_C)//500)
optimal_C = 0
...
def Collatz(start_number):
    ...
    integer_data = { "collatz_sequence": collatz_sequence, "collatz_length": len(collatz_sequence), "max_collatz": max(collatz_sequence), "sequence_sum": sum(collatz_sequence)}
...
def length_of_list (start_number):
    if start_number <= 0:
        print("The value must be a positive integer.")
...
def highest_number (start_number):
    if start_number <= 0:
        print("The value must be a positive integer.")
...
def Collatz_winner(number_list):
    ...
    for number in number_list:
        length = length_of_list(number)
        highest_value = highest_number(number)

        if length >= maximum_length and highest_value >= maximum_value:
            winner = number
            maximum_length = length
            maximum_value = highest_value
