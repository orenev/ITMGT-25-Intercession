...
def bin_to_dec(binary_string):
    ...
    for i in range (len(binary_string)):
        index = int(binary_string[i])
        power = len(binary_string) - i - 1
        number += index * (2**power)
...
def telephone_cipher(message):
    ...
    for i in range (len(message)):
        specific_letter = message[i]
        corresponding_digit = encoder_dict[specific_letter]
        output.append(corresponding_digit)

        if i + 1 < len(message): 
            next_letter = message [i + 1] 
            next_digit = encoder_dict[next_letter] 
            if corresponding_digit [-1] == next_digit [0]:
                output.append ("_")
...
def telephone_decipher(telephone_string):
    ...
    ----------
    message: str
        the numerical string typed in a Telephone Keypad__-
        with underscores demarcating characters who share the same key
    ...
    result = [] 
    current_group = telephone_string[0] 
    for char in telephone_string[1:]:
    ...
    for i in result:
        if i == "_": 
            ...
            corresponding_letter = decipher_dict[i]
            output.append (corresponding_letter)
    return ''.join(output)
            
telephone_decipher("83377778")
